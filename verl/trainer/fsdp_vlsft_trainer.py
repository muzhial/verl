...
from verl.utils.dataset import SFTDataset, SFTVLDataset, DataCollatorForSupervisedDataset, VLTensorDict
...
def _setup_full_tuning(
    ...
    forbidden_modules = get_forbidden_modules(freeze_vision_tower, freeze_multi_modal_projector, freeze_language_model)
...
class FSDPSFTTrainer:
    ...
    def _build_dataloader(self, train_dataset, val_dataset):
        ...
        if isinstance(self.train_dataset, SFTVLDataset):
    ...
    def _build_model_optimizer(self):
        ...
        _setup_full_tuning(
            self.model,
            self.config.model.freeze_vision_tower,
            self.config.model.freeze_multi_modal_projector
        )
        ...
        #         if getattr(param, "placements") == (Shard(0),):
        #             print(param.to_local().size())

        log_gpu_memory_usage("After FSDP wrapping", logger=logger)
        # params = filter(lambda p: p.requires_grad, self.fsdp_model.parameters())
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        pixel_values = torch.cat(batch["pixel_values"], dim=0).to(self.device_name)
        image_grid_thw = batch["image_grid_thw"].to(self.device_name)
        # loss_mask = batch.pop("loss_mask")[:, :-1].reshape(-1).to(self.device_name)
        loss_mask = (labels != -100)[:, 1:].reshape(-1).to(self.device_name)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                if position_ids.dim() == 3:
                    input_ids_rmpad_sliced, position_ids_rmpad_padded, pad_size = ulysses_pad(
                        input_ids_rmpad,
    ...
    def training_step(self, batch: Union[TensorDict, VLTensorDict]):
        ...
        grad_norm = grad_norm.to_local()  # grad_norm is a DTensor
        if is_cuda_available:
        ...
        return {"train/loss": step_loss.detach().item(), "train/lr": lr, "train/grad_norm": grad_norm.detach().item()}
    ...
    def fit(self):
        ...
        if self.config.trainer.profile:
            prof = torch.profiler.profile(
                ...
                on_trace_ready=torch.profiler.tensorboard_trace_handler(f"./logs/{self.config.trainer.profile_name}"),
                record_shapes=True,
                profile_memory=True,
            )
            prof.start()
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                data = VLTensorDict(**data).to(self.device_name)
                # TODO: add video data
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        #     self.device_name
                        # )
                        val_data = VLTensorDict(**val_data).to(self.device_name)
...
def create_sft_dataset(data_paths, data_config, tokenizer, processor):
    ...
    # Then check if multi-turn dataset should be used
        ...
        dataset_cls = SFTVLDataset
    # Default to single-turn dataset
    else:
        dataset_cls = SFTDataset

    # Create datasets based on the selected class
    dataset = dataset_cls(data_paths, tokenizer=tokenizer, config=data_config, processor=processor)
    return dataset
